...
def uNFWtr_fct(rbin,cvir,t,Mvir,param):
    ...
    rvir = (3.0*Mvir/(4.0*np.pi*param.code.deltavir*rhoc_of_z(param)))**(1.0/3.0)
    x = cvir*rbin/rvir
...
def rhoNFW_fct(rbin,cvir,Mvir,param):
    ...
    """
    rvir = (3.0*Mvir/(4.0*np.pi*param.code.deltavir*rhoc_of_z(param)))**(1.0/3.0)
    rho0 = param.code.deltavir*rhoc_of_z(param)*cvir**3.0/(3.0*np.log(1.0+cvir)-3.0*cvir/(1.0+cvir))
    x = cvir*rbin/rvir
...
def profiles(rbin,Mvir,cvir,cosmo_corr,cosmo_bias,param):
    ...
    tau  = eps*cvir
    rvir = (3.0*Mvir/(4.0*np.pi*param.code.deltavir*rhoc_of_z(param)))**(1.0/3.0)
    # r500 = r500_fct(rvir, cvir, param.code.deltavir)
    ...
    rho0NFWtr = param.code.deltavir*rhoc_of_z(param)*cvir**3.0/(3.0*mNFWtr_fct(cvir,tau))
